...
int the_p_count = 0;
int the_key1,the_val1,the_key2,the_val2;

int the_d_count = 0;
int the_d_key1,the_d_key2;

int the_g_count = 0;
int the_g_key1,the_g_key2;
...
//structure to hold parameters for multithreading
typedef struct {
        lsm *ptr_c0;
        lsm *run;
        lsm *ptr_lsm_purge;
        lsm_meta *ptr_lsm_meta;
        int the_key;
        int the_value;
        bloom_t b1;
} multi_lsm;
...
void *mt_get_func(multi_lsm_lookup *multi){
    
    //run lsm_get function
    lsm_lookup(multi->ptr_c0,multi->run,multi->ptr1,multi->ptr2,multi->ptr_lsm_meta,multi->b1,multi->b2,multi->b3,multi->b4,multi->b5,multi->b6,multi->the_key);
    return NULL;
...
void *mt_delete_func(multi_lsm_delete *multi){
    
    //run lsm_get function
    lsm_delete (multi->ptr_c0,multi->the_key);
    return NULL;
...
int main(int argc, char **argv) 
{
    ...
    //multi_lsm *multi;
    //multi=malloc(50*array_size*sizeof(lsm));
    //multi=malloc(16*sizeof(multi_lsm));

    //multi->ptr_c0 = c0;
    //multi->ptr_lsm_meta = lm;
    /////////////////////////////////////////////
                ...
                switch(c){
                    case 'p': //put functions runs here
                        ...
                        if(the_p_count == 2){       
                            ...
                            multiX=malloc(7*sizeof(multi_lsm));
                            multiX->ptr_c0 = c0;
                            multiX->run = run;
                            multiX->ptr_lsm_purge = lsm_purge;
                            multiX->ptr_lsm_meta = lm;
                            multiX->b1 = bloom1;
                            multiX->the_key = the_key1;
                            multiX->the_value = the_val1;
                    ...
                    case 'r': //range function runs here
                        ...
                        if(the_r_count == 2){
                            ...
                            multi4=malloc(13*sizeof(multi_lsm_range));
                            multi4->ptr_c0 = c0;
                            multi4->the_min_key = the_r_minKey1;
                            multi4->the_max_key = the_r_maxKey1;
                            multi4->run = run;
                            multi4->ptr1 = ptr1;
                            multi4->ptr2 = ptr2;
                            multi4->ptr_lsm_meta = lm;
                            multi4->b1 = bloom1;
                            multi4->b2 = bloom2;
                            multi4->b3 = bloom3;
                            multi4->b4 = bloom4;
                            ...
                            //create 2nd thread that executes mt_put_func
                            if(pthread_create(&threadR,NULL,mt_range_func,multi4)){
                                fprintf(stderr, "Error creating thread\n");
                                return 1;
                            }   

                            lsm_range(c0,run,ptr1,ptr2,lm,bloom1,bloom2,bloom3,bloom4,bloom5,bloom6,the_r_minKey2,the_r_maxKey2);

                            //wait for second thread to finish///////////////////////////
                            if(pthread_join(threadR,NULL)){
                                fprintf(stderr, "Error joining thread\n");
                                return 2;
                            }
                ...
                switch(c){
                    case 'g': //get function runs here
                        ...
                        if(the_g_count == 2){
                            ...
                            multi_lsm_lookup *multi3;

                            multi3=malloc(12*sizeof(multi_lsm_lookup));
                            multi3->ptr_c0 = c0;
                            multi3->the_key = the_g_key1;
                            multi3->run = run;
                            multi3->ptr1 = ptr1;
                            multi3->ptr2 = ptr2;
                            multi3->ptr_lsm_meta = lm;
                            multi3->b1 = bloom1;
                            multi3->b2 = bloom2;
                            multi3->b3 = bloom3;
                            multi3->b4 = bloom4;
                            ...
                            //create 2nd thread that executes mt_put_func
                            if(pthread_create(&threadG,NULL,mt_get_func,multi3)){
                                fprintf(stderr, "Error creating thread\n");
                                return 1;
                            }   

                            lsm_lookup(c0,run,ptr1,ptr2,lm,bloom1,bloom2,bloom3,bloom4,bloom5,bloom6,the_g_key2);

                            //wait for second thread to finish///////////////////////////
                            if(pthread_join(threadG,NULL)){
                                fprintf(stderr, "Error joining thread\n");
                                return 2;
                            }
                    ...
                    case 'd': //d function runs here
                        ...
                        if(the_d_count == 2){
                            ...
                            multi_lsm_delete *multi2;

                            multi2=malloc(2*sizeof(multi_lsm_delete));
                            multi2->ptr_c0 = c0;
                            multi2->the_key = the_d_key1;

                            pthread_t threadD;

                            //create 2nd thread that executes mt_put_func
                            if(pthread_create(&threadD,NULL,mt_delete_func,multi2)){
                                fprintf(stderr, "Error creating thread\n");
                                return 1;
                            }   

                            lsm_delete (c0, the_d_key2);

                            //wait for second thread to finish///////////////////////////
                            if(pthread_join(threadD,NULL)){
                                fprintf(stderr, "Error joining thread\n");
                                return 2;
                            }
    ...
    printf("LOOKUP 3: %d\n",lsm_lookup (c0,run,ptr1,ptr2,lm,bloom1,bloom2,bloom3,bloom4,bloom5,bloom6,43));

    printf("EEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEE THE FPR for bloom1 IS: %d\n",lm[0].bloom_miss[1]/lm[0].bloom_hit[1]);
    printf("EEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEE THE FPR for bloom2 IS: %d\n",lm[0].bloom_miss[2]/lm[0].bloom_hit[2]);*/
